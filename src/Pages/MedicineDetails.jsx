...
export default function MedicineDetails() {
  ...
  const { medicine, loading, error } = useMedicineDetails(id);
  ...
  if (error || !medicine) {
    return (
      <div className="status-message error">
        ...
        <p>The requested medicine could not be found.</p>
  ...
  const manufacturer =
    medicine.openfda?.manufacturer_name?.[0] || "Unknown Manufacturer";

  const dosageForm = medicine.openfda?.dosage_form?.[0] || "Not Available";

  const route = medicine.openfda?.route?.[0] || "Not Available";

  const purpose = medicine.purpose?.[0] || "Not Available";

  const warnings =
    medicine.warnings?.[0] || "No warning information available.";

  const directions =
    medicine.directions?.[0] || "No direction information available.";
      ...
      <div className="medicine-details-container">
        ...
        <div className="medicine-details-header">
          ...
          <div className="details-title">
            <p className="details-label">MEDICINE INFORMATION</p>
